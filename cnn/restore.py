...
# 设置CSV文件路径
csv_file = "objects_single_part1.csv"
...
def restore_images_from_csv(csv_file, num_images=100):
    ...
    with open(csv_file, mode='r', newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            label = int(row['label'])
            pixel_values_H = [int(row[f'pixel_H{i}']) for i in range(1, 12545)]
            pixel_values_S = [int(row[f'pixel_S{i}']) for i in range(1, 12545)]
            pixel_values_V = [int(row[f'pixel_V{i}']) for i in range(1, 12545)]

            # 将H, S, V通道的数据合并成一个图像
            h_channel = np.array(pixel_values_H, dtype=np.uint8).reshape((112, 112))
            s_channel = np.array(pixel_values_S, dtype=np.uint8).reshape((112, 112))
            v_channel = np.array(pixel_values_V, dtype=np.uint8).reshape((112, 112))
